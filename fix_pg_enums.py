...
# We need to rename existing enum values from uppercase to lowercase.
# Postgres requires ALTER TYPE ... RENAME VALUE ... TO ...
...
for t in types_to_fix:
    ...
    for val in values:
        if val != val.lower():
            sql = f"ALTER TYPE {t} RENAME VALUE '{val}' TO '{val.lower()}';"
            try:
                print(sql)
                cur.execute(sql)
