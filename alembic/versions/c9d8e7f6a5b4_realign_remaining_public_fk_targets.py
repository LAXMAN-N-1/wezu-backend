"""Realign remaining public foreign keys to public schema

Revision ID: c9d8e7f6a5b4
Revises: b7c1d2e3f4a5
Create Date: 2026-04-02 08:25:00
"""
...
def _constraint_rows(conn: sa.engine.Connection) -> Iterable[dict[str, object]]:
    query = sa.text(
        ...
        JOIN pg_class public_parent ON public_parent.relname = parent.relname
        JOIN pg_namespace public_ns
            ON public_ns.oid = public_parent.relnamespace
...
def upgrade() -> None:
    ...
    for row in _constraint_rows(bind):
        ...
        fk_sql = (
            ...
            f"REFERENCES public.{_quote_ident(parent_table)} ({parent_columns}) "
