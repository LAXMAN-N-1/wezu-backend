    ...
    def process_telemetry(session: Session, device: IoTDevice, data: dict):
        ...
        if "lat" in data and "lng" in data:
            gps_log = GPSTrackingLog(
                ...
                is_geofence_violation=False # Check service later
            )
            session.add(gps_log)
        ...
        if "voltage" in data:
            health_log = BatteryHealthLog(
                battery_id=device.battery_id,
                charge_percentage=data.get("soc", 0), # State of Charge
                health_percentage=data.get("soh", 100), # State of Health
                voltage=data.get("voltage", 0),
                current=data.get("current", 0),
                temperature=data.get("temp", 0),
                cycle_count=data.get("cycles", 0)
            )
            session.add(health_log)
            
            # Update Battery Current State
            ...
            if battery:
                ...
                battery.temperature = data.get("temp", battery.temperature)
                battery.voltage = data.get("voltage", battery.voltage)
                session.add(battery)
