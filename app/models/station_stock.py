...
class StationStockConfig(SQLModel, table=True):
    ...
    station_id: int = Field(foreign_key="stations.stations.id", unique=True, index=True)
    ...
    updated_by: Optional[int] = Field(foreign_key="core.users.id", nullable=True)
...
class ReorderRequest(SQLModel, table=True):
    ...
    created_by: Optional[int] = Field(foreign_key="core.users.id", nullable=True)
...
class StockAlertDismissal(SQLModel, table=True):
    ...
    station_id: int = Field(foreign_key="stations.stations.id", index=True)
    reason: str
    dismissed_by: int = Field(foreign_key="core.users.id")
