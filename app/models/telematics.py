...
class TelemeticsData(SQLModel, table=True):
    ...
    # Advanced Data
    error_codes: Optional[dict] = Field(default=None, sa_column=sa.Column(JSON().with_variant(JSON, "postgresql")))
    raw_payload: Optional[dict] = Field(default=None, sa_column=sa.Column(JSON().with_variant(JSON, "postgresql")))
