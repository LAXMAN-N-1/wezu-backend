...
class BankRejectRequest(BaseModel):
    reason: str

@router.get("/bank-approvals")
def get_pending_bank_approvals(
    current_user: Any = Depends(deps.get_current_active_admin),
    db: Session = Depends(get_db)
):
    """Admin: get all dealers with pending bank account updates"""
    dealers = db.exec(select(DealerProfile)).all()
    pending = []
    for d in dealers:
        if d.bank_details and d.bank_details.get("status") == "PENDING_APPROVAL":
            pending.append(d)
    
    return {"status": "success", "data": pending}

@router.post("/{id}/bank-approve")
def approve_dealer_bank(
    id: int,
    current_user: Any = Depends(deps.get_current_active_admin),
    db: Session = Depends(get_db)
):
    """Admin: approve pending bank details for a dealer"""
    dealer = db.get(DealerProfile, id)
    if not dealer:
        raise HTTPException(status_code=404, detail="Dealer not found")
        
    if not dealer.bank_details or dealer.bank_details.get("status") != "PENDING_APPROVAL":
        raise HTTPException(status_code=400, detail="No pending bank approval found")
        
    updated_bank = dict(dealer.bank_details)
    updated_bank["status"] = "VERIFIED"
    updated_bank["verified"] = True
    
    dealer.bank_details = updated_bank
    db.add(dealer)
    db.commit()
    
    return {"status": "success", "dealer_id": id, "bank_status": "VERIFIED"}

@router.post("/{id}/bank-reject")
def reject_dealer_bank(
    id: int,
    request: BankRejectRequest,
    current_user: Any = Depends(deps.get_current_active_admin),
    db: Session = Depends(get_db)
):
    """Admin: reject pending bank details for a dealer"""
    dealer = db.get(DealerProfile, id)
    if not dealer:
        raise HTTPException(status_code=404, detail="Dealer not found")
    ...
    updated_bank = dict(dealer.bank_details)
    updated_bank["status"] = "REJECTED"
    updated_bank["verified"] = False
    updated_bank["rejection_reason"] = request.reason
    
    dealer.bank_details = updated_bank
    db.add(dealer)
    db.commit()
    
    return {"status": "success", "dealer_id": id, "bank_status": "REJECTED", "reason": request.reason}
