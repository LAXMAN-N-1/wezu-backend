...
def seed_analytics_data():
    with Session(engine) as session:
        ...
        for i in range(50):
            ...
            if not user:
                ...
                user = User(
                    ...
                    is_active=True,
                    created_at=datetime.utcnow() - timedelta(days=created_days_ago) # Simulated creation date
                )
                session.add(user)
                session.flush()
            users.append(user)
        ...
        for i in range(300): # 300 rentals overall
            ...
            start_time = datetime.utcnow() - timedelta(days=days_ago, hours=random.randint(1, 10))
            ...
            if status == "completed":
                ...
                total_price = round(duration_hours * 50.0, 2) # Example 50 per hour
            else:
                end_time = None
                total_price = 0.0
            ...
            rental = Rental(
                ...
                pickup_station_id=station_start.id,
